...
voice_switch = 0
vol_oah = 0.5 * voice_switch
vol_aah = 2 * voice_switch
vol_low_voice = 4 * voice_switch
  ...
  if vol_glass>0
    with_fx :reverb, mix: 0.5, room: 1 do
      with_fx :hpf, mix: glass_hpf_mix, cutoff: scale_linear($total_beats,16,90,50) do
        #with_fx :hpf, mix: 1, cutoff: 50 do
        4.times do
...
live_loop :snr do
  cue :glass
  if vol_snr >0
    with_fx :hpf, mix: 1, cutoff: 110 do
...
live_loop :hh do
  cue :glass
  #with_fx :panslicer, mix: 0.5, phase: 1/32.0 do
  4.times do
    sleep 1/4.0
...
live_loop :ariana do
  ...
  if vol_low_voice>0 or vol_std_voice>0 or vol_aah>0 or vol_oah>0
    1.times do
